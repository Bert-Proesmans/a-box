...
pkgs.runCommand "agent-vm-device1-v0.squashfs"
  ...
  ''
    ...
    # B3's pid1-init mounts proc/sysfs/devtmpfs/tmpfs onto /proc /sys /dev
    # /tmp at boot. The root filesystem is this read-only squashfs, so
    # those mount targets can't be created at runtime - they must already
    # exist as empty directories in the image.
